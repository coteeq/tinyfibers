...
int main() {
  ...
  RunScheduler([]() {
    ...
    JoinHandle child = Spawn([]() {
      ...
      // Перепланируем текущий файбер в конец очереди планировщика
      self::Yield();
    });
    // Блокируем текущий файбер до завершения дочернего
    child.Join();
    std::cout << "Child finished" << std::endl;
  });
  // Вызов RunScheduler завершается когда не останется готовых исполняться файберов
  return 0;
}
```
